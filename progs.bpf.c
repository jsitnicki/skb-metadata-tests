...
#define bpf_print(fmt, ...) \
	bpf_stream_printk(BPF_STREAM_STDOUT, fmt, ##__VA_ARGS__)
#define bpf_eprint(fmt, ...) \
	bpf_stream_printk(BPF_STREAM_STDERR, fmt, ##__VA_ARGS__)
...
static int dump_meta_dynptr(struct __sk_buff *skb)
{
	...
	bpf_print("%pI6\n"
		  ...
		  "%pI6\n"
		  "%pI6\n",
		  &md[0x00], &md[0x10], &md[0x20], &md[0x30], &md[0x40],
		  &md[0x50], &md[0x60], &md[0x70], &md[0x80], &md[0x90],
		  &md[0xa0], &md[0xb0]);

	return 0;
}

static int dump_meta_pktptr(struct __sk_buff *skb)
{
	__u8 *data = (void *)(long)skb->data;
	__u8 *md = (void *)(long)skb->data_meta;
...
static int tcx_dump_meta_dynptr(struct __sk_buff *ctx)
{
	int ret;

	ret = dump_meta_dynptr(ctx);
	if (ret)
		bpf_eprint("error: dump_meta_dynptr -> %d\n", ret);
...
int tcx_dump_meta_pktptr(struct __sk_buff *ctx)
{
	...
	ret = dump_meta_pktptr(ctx);
	if (ret)
		bpf_eprint("error: dump_meta_pktptr -> %d\n", ret);
...
{
	bpf_stream_printk(BPF_STREAM_STDOUT, "hello stdout\n");
	bpf_stream_printk(BPF_STREAM_STDERR, "hello stderr\n");
